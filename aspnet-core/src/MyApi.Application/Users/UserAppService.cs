...
namespace MyApi.Users
{
    ...
    public class UserAppService : AsyncCrudAppService<User, UserDto, long, PagedUserResultRequestDto, CreateUserDto, UserDto>, IUserAppService
    {
        ...
        [AbpAuthorize(AdministrativePermissionActions.Administrative_User_Edit)]
        public override async Task<UserDto> CreateAsync(CreateUserDto input)
        {
            CheckCreatePermission();
            input.UserName = input.Name;
            input.Password = "myApi@01";
            input.FirstLogin = true;
        ...
        protected override IQueryable<User> CreateFilteredQuery(PagedUserResultRequestDto input)
        {
            ...
            return Repository.GetAllIncluding(x => x.Roles).WhereIf(!string.IsNullOrEmpty(input.Keyword), x => x.Name.Contains(input.Keyword));
        }
