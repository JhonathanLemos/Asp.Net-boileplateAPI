...
namespace MyApi.Customers
{
    ...
    public class CustomerAppService : AsyncCrudAppService<Customer, CustomerDto, long, GetAll, CreateCustomerDto>, ICustomerAppService
    {
        ...
        public CustomerAppService(IRepository<Customer, long> repository, Abp.ObjectMapping.IObjectMapper objectMapper, IProductAppService productAppService) : base(repository)
        {
        ...
        {
            return Repository.GetAllIncluding(x => x.Produtos).WhereIf(!string.IsNullOrEmpty(input.Keyword), x => x.Nome.Contains(input.Keyword));
        }
        ...
        {
            return await _productAppService.GetProductsByCustomerId(input);
        }
    }
}
